...
EASY używa istniejącego budowania kandydatów i czterech faultów systemd w trzech środowiskach. Adapter do V3 dodaje na drugim hoście usługę obserwującą jedną bezpośrednią zależność oraz kontrolowany root cause. Zachowuje objectives, hints i reference solution z istniejących template’ów. MEDIUM korzysta z pięciu kuratorowanych fixture’ów: firewalld, pakiety, SELinux, DNS/NetworkManager i zewnętrzny firewall. Każdy wariant ma jeden fault; HARD i dowolne kombinacje świata pozostają niedostępne. Seed zmienia lokalnie nazwy hostów i odpowiadające im odwołania w reference solution, podpowiedziach i raporcie. Pliki, logi, pakiety, reguły sieciowe i rozwiązanie zawsze pochodzą z kodu ShellForge. Nawet lokalnie zmaterializowany draft przechodzi pełną walidację i replay.
